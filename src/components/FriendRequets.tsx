...
interface FriendRequetsProps {
  incomingFriendRequests: IncomingFriendRequest[];
  sessionId: string;
}

const FriendRequets: FC<FriendRequetsProps> = ({ incomingFriendRequests, sessionId }) => {
  const router = useRouter();
  const [firendRequests, setFirendRequests] = useState<IncomingFriendRequest[]>(incomingFriendRequests);

  const acceptFriend = async (senderId: string) => {
    await axios.post("/api/friends/accept", { id: senderId });

    setFirendRequests(prev => prev.filter(request => request.senderId !== senderId));
